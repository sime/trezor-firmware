...
class _MessageTypeMeta(type):
    def __init__(cls, name: str, bases: tuple, d: dict) -> None:
        super().__init__(name, bases, d)  # pyright: ignore [Expected 1 positional argument]
        if name != "MessageType":
            cls.__init__ = MessageType.__init__  # pyright: ignore ["__init__" is obscured by a declaration of the same name;;Cannot assign member "__init__" for type "_MessageTypeMeta"]
