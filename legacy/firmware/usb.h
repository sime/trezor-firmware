...
/*
 * Setting this value to 0 will limit the protobuf messages `usbPoll` and
 * `usbSleep` can handle to a few defined in `msg_read_tiny`.
 *
 * Setting to 0 is meant to prevent infinite recursion when you need to read a
 * message while being called from FSM.
 *
 * Setting to 1 allows processing all messages.
 */
 ...
 * you must use `usbTiny(0)` before and `usbTiny(oldTinyValue)` after, otherwise
 * there is possibility of stack exhaustion.
 */
