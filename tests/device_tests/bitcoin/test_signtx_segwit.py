...
def test_send_p2sh(client: TrezorClientDebugLink):
...
def test_send_p2sh_change(client: TrezorClientDebugLink):
    inp1 = messages.TxInputType(
        address_n=parse_path("49h/1h/0h/1/0"),
        # 2N1LGaGg836mqSQqiuUBLfcyGBhyZbremDX
        amount=123_456_789,
...
def test_testnet_segwit_big_amount(client: TrezorClientDebugLink):
    ...
    out1 = messages.TxOutputType(
        address="2Mt7P2BAfE922zmfXrdcYTLyR7GUvbwSEns",  # seed allallall, bip32: 49h/1h/0h/0/1, script type:p2shsegwit
        amount=2 ** 32 + 1,
        script_type=messages.OutputScriptType.PAYTOADDRESS,
    )
    with client:
...
def test_send_multisig_1(client: TrezorClientDebugLink):
    # input: 338e2d02e0eaf8848e38925904e51546cf22e58db5b1860c4a0e72b69c56afe5

    nodes = [
        btc.get_public_node(
            client, parse_path(f"49h/1h/{i}h"), coin_name="Testnet"
        ).node
        for i in range(1, 4)
    ]
    ...
    inp1 = messages.TxInputType(
        address_n=parse_path("49h/1h/1h/1/0"),
    ...
    with client:
        client.set_expected_responses(excpected_responses)
    ...
    assert_tx_matches(
        serialized_tx,
        segwit_hash="8a64e80b5a6e9e5cfc94e1e0fe4ff59310f27a7079e1ed80e59f7e61cd5b8879",
...
def test_attack_change_input_address(client: TrezorClientDebugLink):
    inp1 = messages.TxInputType(
        address_n=parse_path("49h/1h/0h/1/0"),
        # 2N1LGaGg836mqSQqiuUBLfcyGBhyZbremDX
        amount=123_456_789,
    ...
    out2 = messages.TxOutputType(
        address_n=parse_path("49h/1h/12h/1/0"),
        script_type=messages.OutputScriptType.PAYTOP2SHWITNESS,
        amount=123_456_789 - 11_000 - 12_300_000,
    )
