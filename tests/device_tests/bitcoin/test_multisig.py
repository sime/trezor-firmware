...
from ...common import MNEMONIC12, assert_tx_matches
...
def test_2_of_3(client: TrezorClientDebugLink):
    ...
    nodes = [
        btc.get_public_node(
            client, parse_path(f"48h/1h/{index}h/0h"), coin_name="Testnet"
        ).node
    ...
    inp1 = messages.TxInputType(
        address_n=parse_path("48h/1h/1h/0h/0/0"),
        amount=1_496_278,
    ...
    inp3 = messages.TxInputType(
        address_n=parse_path("48h/1h/3h/0h/0/0"),
        amount=1_496_278,
        prev_hash=TXHASH_6b07c1,
        prev_index=0,
        script_type=messages.InputScriptType.SPENDMULTISIG,
...
def test_15_of_15(client: TrezorClientDebugLink):
    ...
    node = btc.get_public_node(
        client, parse_path("48h/1h/1h/0h"), coin_name="Testnet"
    ).node
    ...
    for x in range(15):
        ...
        inp1 = messages.TxInputType(
            address_n=parse_path(f"48h/1h/1h/0h/0/{x}"),
            amount=1_476_278,
...
def test_attack_change_input(client: TrezorClientDebugLink):
    ...
    NOTE: The goal of this test is to replicate the "Malicious change in mixed transactions" attack in
    ...
    Previously this test was making sure that MatchChecker's check_input() function
    catches the change in the account type. After scriptPubKey verification was introduced,
    the attack is caught earlier by the scriptPubKey mismatch.
    """
