...
def test_non_segwit_segwit_inputs(client):
    ...
    assert_tx_matches(
        serialized_tx,
        segwit_hash="9706ed37f3d8b1f3b2f5eb2bafdf1d15411ebe3d1a3569f1b0b80d3b417acd73",
...
def test_segwit_non_segwit_inputs(client):
    ...
    assert_tx_matches(
        serialized_tx,
        segwit_hash="4003c06c172c7db3d597d4c3820b31264aaaf773e85ef0e73ff957c3346e0661",
...
def test_segwit_non_segwit_segwit_inputs(client):
    ...
    assert_tx_matches(
        serialized_tx,
        segwit_hash="e38af2d1863adc084772936391961bce305506c94f073b5fd1e44fe78fa5ba1b",
...
def test_non_segwit_segwit_non_segwit_inputs(client):
    ...
    inp3 = messages.TxInputType(
        address_n=parse_path("44h/1h/1h/0/0"),  # msUqRgCWS7ryuFcF34EaKTrsTe3xHra128
        amount=10_000,
    ...
    assert_tx_matches(
        serialized_tx,
        segwit_hash="12340ac91bdf148707ea0e65dba44986591f60bd535347419e95745cbc889dc8",
